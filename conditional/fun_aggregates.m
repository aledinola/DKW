...
% NOTE:
% We compute two exit rates
% - exit_rate: exit rate of all small firms
% - exit_rate_emp: exit rate of small firms excluding microfirms, where
% microfirms are defined as those with employment size less than
% par.emp_min.
% We save both version in the output agg.
...
min_labor = zeros(par.nk,par.nx); % minimum labor requirement. Only used for conditional grant - otherwise set to zero.
...
[entry_cost,output_small,liq,L_small,cost_adj,exit,entry] = sub_aggregates_onestep(mu,mu_active,...
     pol_kp_ind,pol_entry,exit_all,phi_dist,wage,mass,A,par,min_labor);

% Normalize by measure of incumbent firms to get the rate:
