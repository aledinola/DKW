...
% This function calls:
%   - sub_mu_onestep or mu_onestep_mex
%------------------------- input checks ----------------------------------%
if isstruct(par)==0
...
% validateattributes(distrib_ss.mu, {'double'}, {'finite', 'nonnan', 'nonempty', 'real',...
%     '>=', 0,'size', [par.nb,par.nx]})
% validateattributes(pol_tran.pol_entry, {'double'}, {'finite', 'nonnan', 'nonempty',...
%     'real', '>=', 0,'<=', 1,'size', [par.nb,par.nx,par.T+1,2]})
%-------------------------------------------------------------------------%
...
%mu_active_init = distrib_ss.mu_active;
...
%                     knext_ind = pol_kp_ind_t(k_c,b_c,x_c);
%                     bopt      = pol_debt_t(k_c,b_c,x_c);
%                     [left_loc,omega] = find_loc(b_grid_t(knext_ind,:)',bopt);
...
%         end
        ...
        mu(:,:,:,t,n_c) = sub_mu_onestep(mu(:,:,:,t-1,n_c),phi_dist_t,pol_kp_ind_t,...
            pol_exit_t,pol_entry_t,left_loc_arr,omega_arr,pi_x,mass_vec(t),psi);
        %mu(:,:,:,t,n_c) = mu_onestep_mex(mu(:,:,:,t-1,n_c),phi_dist_t,int32(pol_kp_ind_t),...
        %    pol_debt_t,pol_exit_t,pol_entry_t,int32(left_loc_arr),omega_arr,...
        %    b_grid_t,pi_x,mass_vec(t),psi);
...
%     warning("MU does not sum to one!")
% end
            ...
            for b_c = 1:nb
                for k_c = 1:nk
                    mu_active(k_c,b_c,x_c,t,n_c)=(1-psi)*(1-pol_exit(k_c,b_c,x_c,t,n_c))*mu(k_c,b_c,x_c,t,n_c) ...
                        + mass_vec(t)*pol_entry(k_c,b_c,x_c,t,n_c)*phi_dist(k_c,b_c,x_c,t,n_c);
                end
            end
        end
    end
