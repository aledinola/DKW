...
% Check if b_grid is equispaced
%if range(b_grid(2:end)-b_grid(1:end-1))>1e-10
%    error('b_grid is not equally spaced: not allowed in this version of the code')
%end
...
elseif par.matrixInv==1
    ...
    %[mu,mu_active,entry_vec,flag_mu] = fun_distrib1_vec(par,sol,b_grid,phi_dist);
else
