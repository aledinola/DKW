...
%   
%4. Calculate profit profit_mat(1:xn,2:T+1)
%   5. Calculate value functions by backward induction val(1:nb,1:nx,2:T+1),val0(1:nb,1:nx,2:T+1);
%       Also calcualte associated policy functions pol_entry,pol_exit,
%       pol_debt for t = 2,...T+1
%   8. ...
...
C_h    =  0.9025*agg_ss.C_agg;%0.95*agg_ss.C_agg; %vpa(0.95*agg_ss.C_agg);
C_l    =  0.9005*agg_ss.C_agg;%0.85*agg_ss.C_agg; %vpa(0.85*agg_ss.C_agg);
C_init = vpa(0.5*(C_h+C_l));

% debug: set C_init to ss value
% C_init = agg_ss.C_agg;
% C_h = C_init+0.1;
% C_l = C_init-0.1;
iter = 0;
...
%%   1. Solve for C_path(2:T+1), wage(1:T+1),KL_ratio(1:T+1), q(1:T+1)
    ...
    for t=1:T+1
        ...
        if t<T+1
            ...
            % debug: set KL ratio to be the ss value
            %path.KL_ratio(t+1) = path.KL_ratio(t);
            if flag<0
