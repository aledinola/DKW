function [] = plot_irf(irf1,SaveDir,do_save,FormatFig)

% DESCRIPTION
% We plot IRFs after solving the model in the transition. IRFs are
% calculated in fun_targets_tran.m and expressed as deviation from
% steady-state.
% INPUTS:
%   "irf1"        Structure with IRF  
...
%% Set names
varNames = {'C_agg','K_agg','K_small','K_corp','mass_small','exit_rate_emp'...
    'liq','exit_rate','exit','entry_rate','Y_agg','Y_corp','output_small','L_agg','L_small','L_corp',...
    'KL_ratio','InvK_corp','InvK','q','w'};

varNamesLabels = {'Consumption';'Agg. capital';'Capital, small firms';'Capital, corp.';'Mass of small firms';'Exit rate (excl. microbus.)';
...
for ii=1:numel(varNames)
    name = varNames{ii}; % cell array of characters
