...
%             end
        ...
        function adj = adjcost(kprime,k,theta,delta)
            ...
            %adj = zeros(size(kprime));
            %adj(kprime~=k) = realmax;
        ...
        function F = fun_profit_l(x,kappa,l_small,c,wage,par)
            % Static profit by small business with productivity x, wage and 
            % as a function of labor. 
            % NOTE: l_small may not be the optimal one. Compare to
            % "fun_profit".
            F = fun.prod_small(x,kappa,l_small,c,par) - wage*l_small;
