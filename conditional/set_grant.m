...
elseif par.grant_flag == 6 % only impacted firms get grant, large grant
    par.Xp  = 2.5/3*5; % increase Xp until we get the same employment saved
    par.eta = par.eta_i*ones(par.nk,par.nx); % eta==eta_i
...
% NOTE: with conditional grant, weights is endogenous since it depends on
% the decision whether to take up the grant or not. weights is computed in
% fun_vfi1_transition and saved in the struct "pol_tran".
