...
% Usage: [sol,b_grid,phi_dist,conv_flag] = fun_vfi(prices,par), where prices/par are struct
%%%%% Overview:
...
if isstruct(prices)==0
    error("Input <prices> in VFI must be a structure!")
end
if isstruct(par)==0
...
while ind<max_iter && errter > tol_vfi_u
    % Note: last argument is do_howard 0/1
...
toc
%V1  % This is V(x)
%validateattributes(V1, {'double'}, {'finite', 'nonnan', 'nonempty', 'real','size', [nk,nx]})

%save V1_grid V1 k_grid
...
for k_c = 1:nk
    ...
    if ~isempty(viable_x)
        ...
        sol=[];b_grid=[];phi_dist=[];
        return
%         x_tilde(k_c) = nk+1; % out of range
%         x_tilde_val(k_c) = x_grid(nx)+1;
    end
end
...
while ind<max_iter && errter > tol_bhat
    ...
    %[B_hat_new,pol_bp_unc] = sub_Bhat_onestep_mex(B_hat,pol_kp_unc,profit_mat,...
    %    x_tilde_val,k_grid,x_grid,q,theta,delta,lambda);
...
%validateattributes(pol_bp_unc, {'double'}, {'finite', 'nonnan', 'nonempty', 'real','size', [nk,nx]})
...
%validateattributes(b_grid, {'double'}, {'finite', 'nonnan', 'nonempty', 'real','size', [nk,nb]})
...
while iter<max_iter && dist > tolerance
    ...
    %    profit_mat,k_grid,b_grid,pi_x,theta,q,delta,psi,do_howard,n_howard);
...
%validateattributes(pol_entry, {'double'}, {'finite', 'nonnan', 'nonempty','real','>=',0,'<=',1,'size', [nk,nb,nx]})
%validateattributes(pol_exit, {'double'}, {'finite', 'nonnan', 'nonempty','real','>=',0,'<=',1,'size', [nk,nb,nx]})
...
%validateattributes(val, {'double'}, {'nonnan', 'nonempty', 'real','size', [nk,nb,nx]})
%validateattributes(pol_debt, {'double'}, {'nonempty', 'real','size', [nk,nb,nx]})
%validateattributes(pol_exit, {'double'}, {'finite', 'nonnan', 'nonempty','>=',0,'<=',1,'real','size', [nk,nb,nx]})
%validateattributes(pol_entry, {'double'}, {'finite', 'nonnan', 'nonempty','real','>=',0,'<=',1,'size', [nk,nb,nx]})
...
% TODO: note that pol_kp_ind is only properly defined for constrained
% firms. For unconstrained firms, pol_kp may be off-grid.
% In fun_distrib1, we should use pol_kp instead of pol_kp_ind!
%pol_kp_ind = double(pol_kp_ind);
