...
%% Make IRFs for targeted grant economy
disp('SLIM TARGETED GRANT ECONOMY:')
% Make IRFs for slim targeted grant economy
if isfile(fullfile('mat','grant_targslim.mat'))
    load(fullfile('mat','grant_targslim.mat'),'irf')
else
    error('File grant_targslim.mat does not exist!')
end

% Compute IRFs in percentage deviation from steady-state
irf_grant_targslim = irf;
clear irf
...
for ii = 1:numel(varNames)
    ...
    %sr_totimp_grant_targeted.(varii) = 100*sum(irf_grant_targeted.(varii)(1:T_sr))/T_lr;
    sr_totimp_grant_targslim.(varii) = 100*sum(irf_grant_targslim.(varii)(1:T_sr))/T_lr;
    ...
    %mr_totimp_grant_targeted.(varii) = 100*sum(irf_grant_targeted.(varii)(T_sr+1:T_mr))/T_lr;
    mr_totimp_grant_targslim.(varii) = 100*sum(irf_grant_targslim.(varii)(T_sr+1:T_mr))/T_lr;
    ...
    %lr_totimp_grant_targeted.(varii) = 100*sum(irf_grant_targeted.(varii)(T_mr+1:T_lr))/T_lr;
    lr_totimp_grant_targslim.(varii) = 100*sum(irf_grant_targslim.(varii)(T_mr+1:T_lr))/T_lr;
    ...
    %net_totimp_grant_targeted.(varii) = sr_totimp_grant_targeted.(varii) + mr_totimp_grant_targeted.(varii) +lr_totimp_grant_targeted.(varii) ;
    net_totimp_grant_targslim.(varii) = sr_totimp_grant_targslim.(varii) + mr_totimp_grant_targslim.(varii) +lr_totimp_grant_targslim.(varii) ;
...
for ii = 1:numel(varNames)
    ...
    set(gca, 'XTick', xt, 'XTickLabel', {'Laissez-Faire' 'Baseline grant' ...
        'Targeted grant' },'fontsize',myfontsize)
    xtickangle(30)
    hL=plot(1:3,net_totimp.(varii),'sg','MarkerSize',15,'MarkerFaceColor','g');
    hold off
    %title(varlabel,'fontsize',myfontsize)
    ylabel('Pandemic Impact (%)','fontsize',myfontsize)
    ax = gca;
    ax.YAxis.Exponent = 0;
    ax.YAxis.FontSize = myfontsize;
    legend({'Q1', 'Q2-Q8', 'Q9-Q40','Average'},'location','northoutside',...
        'fontsize',myfontsize,'NumColumns',4)
    %legend('boxoff')
