...
Paper title: "Rescue Policies for Small Businesses During the Covid-19
Recession".
...
FIGURES:
    ...
    To compute forced exit in the transition, run "compute_forced_exit".
...
Instructions to compile MEX files
- see README.txt
...
disp('Replication of Di Nola, Kaas and Wang (2023)')
disp(' ')
...
% 4 = welfare maximization of transition by changing grant level

par.grant_flag =1; % 0 = no grant; 1 = baseline grant (uniform); 2 = targeted grant by size
...
elseif par.do_calib == 1
    ...
    % Save workspace in subfolder \mat
    if do_save==1
        val_tran                  = pol_tran.val(:,:,:,1:5,:); %(k,b,x,t,nn)
        ...
        clearvars -except par agg agg_tran b_grid data_mom data_mom_trans ...
            prices path model_mom_trans data_mom_trans irf val_tran pol_tran ...
...
elseif par.do_calib == 3
    ...
    %LB = bounds_shocks(:,1);
    %UB = bounds_shocks(:,2);
    %[x,~,exitflag] = fminsearchcon(f,X0,LB,UB,[],[],[],options);
