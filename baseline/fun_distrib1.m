...
% PURPOSE: 
% fun_distrib1 computes the stationary distribution mu_0(k,b,x), called mu 
% in the code, and the distribution of active firms, mu_active(k,b,x).
% INPUTS: 
% par:      structure
% sol:      structure
% b_grid:   (nk,nb) numeric array
% phi_dist: (nk,nb,nx) numeric array
% OUTPUTS:
% mu         stationary distribution,      dim: (nk,nb,nx)
% mu_active  distribution of active firms, dim: (nk,nb,nx)
% entry_vec
% flag_mu    Convergence flag
% dist       Scalar, real(8)
% iter       Scalar, integer
% This function calls:
